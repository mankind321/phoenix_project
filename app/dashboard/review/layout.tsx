  ...
  useEffect(() => {
    ...
    const markOffline = async () => {
      try {
        ...
        console.warn("⚠ Failed to set offline:", err);
      }
    };

    // 1️⃣ Mark offline if session becomes unauthenticated
    if (["unauthenticated"].includes(status)) {
      markOffline();
    }

    // 2️⃣ Mark offline on browser/tab close
    window.addEventListener("beforeunload", markOffline);

    // 3️⃣ Auto-offline after 10 min of inactivity
